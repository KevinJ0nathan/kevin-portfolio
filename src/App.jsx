  ...
  return (
    <>
      <div className="flex h-screen overflow-hidden bg-d-background-primary font-sans text-white">
        ...
        <div
          {...leftSidebarSwipe}
          className={`fixed inset-y-0 left-0 z-50 flex w-[300px] transition-transform duration-300 md:hidden ${isLeftMenuOpen ? "translate-x-0" : "-translate-x-full"}`}
        >
          {/* Mobile Server List */}
        ...
        <div
          {...rightSidebarSwipe}
          className={`fixed inset-y-0 right-0 z-50 w-[240px] bg-[#2f3136] transition-transform duration-300 lg:hidden ${isRightMenuOpen ? "translate-x-0" : "translate-x-full"}`}
        >
          <MemberList />
        </div>
      </div>
